...
const Orders = ({ token }) => {
  ...
  const statusHandler = async (event, orderId) => {
    ...
    try {
      ...
      console.log(error);
      toast.error(response.data.message) // Show error message from catch block
      ...
      <div>
        {
          orders.map((order, index) => (
            <div className='grid grid-cols-1 sm:grid-cols-[0.5fr_2fr_1fr] lg:grid-cols-[0.5fr_2fr_1fr_1fr_1fr] gap-3 item-start border-2 border-gray-200 p-5 md:p-8 my-3 md:my-4 text-xs sm:text-sm text-gray-700' key={index}>
              ...
              <select onChange={(event) => statusHandler(event, order._id)} value={order.status} className='text-xs sm:text-sm border rounded px-2 py-0.5 font-normal'>
